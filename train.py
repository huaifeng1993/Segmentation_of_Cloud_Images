    ...
    def train(self, train_loader, val_loader, loss_function, num_epochs):
        ...
        for epoch in range(self.epoch, num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                print('{} Loss: {:.4f} P:{:.4} R:{:.4} F1:{:.4} Mr:{:.4} Acc:{:.4} mIOU:{:.4}'.format(
                    phase, epoch_loss, P, R, F1, Mr,Acc,mIOU))

                writer.add_scalar('{}_loss'.format(phase), epoch_loss, epoch)
              
                writer.add_scalar('{}_P'.format(phase), P, epoch)
                writer.add_scalar('{}_R'.format(phase), R, epoch)
                writer.add_scalar('{}_F1'.format(phase), F1, epoch)
                writer.add_scalar('{}_Mr'.format(phase), Mr, epoch)
                writer.add_scalar('{}_mIOU'.format(phase), mIOU, epoch)
